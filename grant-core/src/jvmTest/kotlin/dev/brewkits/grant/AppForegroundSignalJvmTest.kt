...
/**
 * Pins the "honest no-op" contract from [AppForegroundSignal]'s KDoc: a plain JVM process has
 * no universal foreground signal, so [AppForegroundSignal.isSupported] must stay `false` and
 ...
 */
class AppForegroundSignalJvmTest {

    @Test
    fun `isSupported is false on the JVM target`() {
