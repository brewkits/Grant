...
- [x] **Verified on a real Android 17 device — and the experiment corrected the diagnosis.** A probe Activity declared with `android:process=":probe"` requested CAMERA without `setLauncher()`. It did not resolve (`NO_RESOLUTION`, 25s), and the new advisory fired correctly. **But the control run — the same Activity in the main process — also did not resolve**, even though the system dialog appeared and Allow was tapped (the permission genuinely flipped to `granted=true`).
- [x] So the process split is **not the cause**, only an aggravating factor. The real defect is in the `GrantRequestActivity` fallback's own completion path: `requestViaActivity()` captures a `CompletableDeferred` *before* launching, while `GrantRequestActivity` completes `pendingResults[requestId]` from its `ActivityResultCallback`. In the observed runs that completion never reached the awaiting caller, in a single process, with the dialog answered. The multi-process case fails for a superset of reasons, which is why the probe found it first.
- [ ] **Follow-up: fix the fallback's completion path.** This is a real single-process bug affecting the default request path (no `setLauncher()`), and it is more serious than the multi-process hazard it was found through. Not fixed in this branch — it was discovered while verifying something else, and a completion-path change to the fallback deserves its own change with its own regression test rather than being folded in here. The advisory added here stays correct and useful; it just is not the whole story.
